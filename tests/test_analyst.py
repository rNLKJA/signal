...
from signalkit.data.nypd import MonthlyRecord
...
def test_ask_citywide(analyst):
    answer = analyst.ask(AnalystQuery(question="overall trend", months=12))
    assert answer.stats.total_complaints > 0
...
def test_ask_filtered(analyst):
    answer = analyst.ask(AnalystQuery(offense="burglary", borough="brooklyn", months=12))
    assert answer.stats.total_complaints > 0
    assert answer.stats.top_offenses[0]["offense"] == "BURGLARY"


def test_every_answer_is_logged(analyst):
    a1 = analyst.ask(AnalystQuery(offense="robbery"))
    a2 = analyst.ask(AnalystQuery(offense="felony assault"))
...
def test_no_match_raises_with_suggestions(analyst):
    ...
    assert "boroughs" in exc.value.suggestions
    assert exc.value.suggestions["boroughs"]
