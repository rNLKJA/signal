...
| `POST /ask/multi` | A compound question over 2–6 offences, answered as a traceable tree: each offence is its own logged, faithfulness-checked sub-decision (`parent_decision_id`), and a parent decision synthesises them. The whole tree is in the tamper-evident chain. |
...
- **Per-step accountability for multi-step answers.** A compound question (`POST /ask/multi`) is decomposed into linked sub-decisions — each its own logged, faithfulness-checked entry pointing back to a parent that synthesises them — and the whole decision tree sits in the tamper-evident chain. Accountability that survives multi-step reasoning is the problem the Five Eyes agentic-AI guidance named hardest; this is a concrete answer to it.
